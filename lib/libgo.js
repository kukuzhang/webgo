...
function equalUser(a,b) {
  ...
  function Game(options) {
    ...
    var defaults = {
      black : null, // Player object
      white : null, // Player object
  ...
  Game.prototype.myColor = function (user) {

    if (equalUser(user,this.black))  { return BLACK; }

    if (equalUser(user,this.white)) { return WHITE; }

    return null;

  };

  Game.prototype.getMoveError = function (move) {
  ...
  Game.prototype.isMoveOk = function (move) {
    ...
    return this.getMoveError(move) ? false : true;
  ...
  Game.prototype.play = function (move) {
    ...
    if (newState.state === 'scoring') { this.getInitialScoring(); }
  ...
  Game.prototype.isPrisoner = function (row,column) {
    ...
    var scorePoint = this.scoreBoard[this.getBoard().point2Index(row,column)] 
  ...
  Game.prototype.markOrUnmarkAsPrisoner = function (row,column) {
    ...
    if (this.isPrisoner(row,column)) {
      ...
      for (var index in group.points) {
        ...
        var containsPrisoner = board.getStoneByIndex(index) != EMPTY;
        this.scoreBoard[index] =
          ...
          group.owner === WHITE &&  containsPrisoner ? BLACK_DEAD : EMPTY;

      }

    }

    console.log('marked',group);
    return group
  ...
  Board.prototype.isPointOnBoard = function (row,column) {

    return this.validCoordinate(row) &&
           this.validCoordinate(column);

  }

  Board.prototype.isMoveOnBoard = function (move) {

    return this.isPointOnBoard(move.row, move.column);
  ...
  Board.prototype.playStone = function (move) {
    ...
    function kill(board,row,column) {
      ...
      if (!board.isPointOnBoard(row,column) ||
        board.getStone(row,column) === EMPTY) {
        return;
      }

      var groupToKill = board.createGroup(row,column);

      if (!groupToKill) { return; }

      if (groupToKill.getLiberties(board) > 0) { return; }
      groupToKill.removeFromBoard(board);
    ...
    kill(newBoard, move.row-1, move.column);
    kill(newBoard, move.row+1, move.column);
    kill(newBoard, move.row, move.column-1);
    kill(newBoard, move.row, move.column+1);
    if (false) {
  ...
  Board.prototype.index2Point = function (index) {

    var row = Math.floor (index / this.boardSize);
    var column = index % this.boardSize;
  ...
  Board.prototype.createScoringGroup = function (index,prisoners) {
    var g = this.createGroupAtIndex(index);
    var neighbours = g.getNeighbours(this);
    var board = this;
    var nArray = Object.keys(neighbours);

    if (nArray.length) {

      var surroundedBy = this.getStoneByIndex(nArray.reduce(function (prev,cur) {
      ...
      console.log('scoring group with', neighbours.length, 'neighbours', surroundedBy);
  ...
  Board.prototype.createGroupAtIndex = function (index) {
    var point = this.index2Point(index);
    return new Group(this,point[0],point[1]);
  };

  Board.prototype.createGroup = function (row,column) {
  ...
  function Group (board, row, column, prisonerColor) {
    ...
    if (this.points[index] !== undefined) { return; }
  ...
  Group.prototype.getNeighbours = function (board) {
    ...
    function tryToAddToNeighbours(points,row,column) {
      ...
      if (points[index] === undefined &&
          board.isPointOnBoard(row,column)) {
