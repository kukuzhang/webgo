(function(exports) {
  ...
  Game.prototype.isPrisoner = function (row,column) {
    ...
    var hash = this.getBoard().point2Hash(row,column);

    for (var i=0; i<this.scoring.groups.length; i++) {

      var g = this.scoring.groups[i];

      if (!g.hashes[hash]) { continue; }

      if (g.prisonerColor) return true;
  ...
  Game.prototype.markOrUnmarkAsPrisoner = function (row,column) {
    ...
    var prisonerColor = this.getBoard().getStone(row,column);
    var hash = this.getBoard().point2Hash(row,column);
    ...
    if (this.isPrisoner(row,column)) {
      ...
      this.scoring.groups = this.scoring.groups.filter( function (g) {
        return !(g.prisonerColor && g.hashes[hash]);
      });
      
    } else {

      var group = this.getBoard().createPrisonerGroup(row,column,prisonerColor);

      this.scoring.groups.push(group);
  ...
  Game.prototype.scorePoint = function (row,column) {
    ...
    var hash = this.getBoard().point2Hash(row,column);
    var groups = this.scoring.groups;
    var prisonerss = this.scoring.prisoners;

    if (!groups) return null;

    for (var i=0; i<groups.length; i++) {

      if (groups[i].hashes[hash]) {

        var owner = groups[i].owner;
        var containsPrisoner = this.getBoard().getStone(row,column) !== EMPTY;
      
        return owner === BLACK && !containsPrisoner ? BLACK_POINT :
               owner === WHITE && !containsPrisoner ? WHITE_POINT :
               owner === BLACK &&  containsPrisoner ? WHITE_DEAD :
               owner === WHITE &&  containsPrisoner ? BLACK_DEAD : null;
  ...
  Board.prototype.getInitialScoringGroups = function () {

    function alreadyInScoringGroups(hash) {

      for (var i = 0; i<scoringGroups.length; i++) {

        if (scoringGroups[i].hashes[hash]) return true;
    ...
    for (var i=0;i<this.length;i++) {
      ...
      for (var j=0;j<this.length;j++) {
        ...
        var hash = this.point2Hash(i,j);

        if (this.stones[i][j] !== EMPTY) { continue; }

        if (alreadyInScoringGroups(hash)) { continue; }

        var group = this.createScoringGroup(i,j);

        if (group.owner) scoringGroups.push(group);
  ...
  function Group (board, row, column, prisonerColor) {
    ...
    if(this.prisonerColor) console.log(row,column,this.content,this.prisonerColor,stone);
