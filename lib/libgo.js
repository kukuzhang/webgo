(function(exports) {
  ...
  Move.prototype.moveTypes = {
    'pass': Pass,
    'resign': Resign,
    'stone': Stone
  };

  Move.prototype.fromString = function (json) {

    if (typeof json === 'string') {json = JSON.parse(json);}
    var Cls = Move.prototype.moveTypes[json.type];
  ...
  function Game(options) {
    ...
    this.moves = this.moves.map(function (m) {
      return Move.prototype.fromString(m);
    });
  ...
  Game.prototype.totalPlayTime = function (color) {
    ...
    for (var i = 0; i < this.moves.length; i++) {
      ...
      //console.log(turnStarted,move.timestamp);
  ...
  Game.prototype.remainingTime = function (color) {

    var total = this.totalPlayTime(color)

    return this.timing.main - total;
  ...
  Game.prototype.play = function (move) {
    ...
    console.log('play',move instanceof Move,move);
    this.assertMoveSanity(move);
    var newBoard = this.getBoard().playStone(move);
    console.log('play',move instanceof Move,move);
    this.moves.push(move);
    var m = this.moves[this.moves.length-1];
    console.log(typeof m, typeof move, this.moves.length,this.moves);
    console.log('played',m instanceof Move,m);
  ...
  Board.prototype.playStone = function (move) {
    ...
    function kill(board,row,column) {
      ...
      if (groupToKill.content !== opposite(move.stone)) { return; }
  ...
  exports.json2Move = Move.prototype.fromString;
