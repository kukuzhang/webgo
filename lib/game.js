...
var PersistentGame = require('./persistency').PersistentGame;
var io;
...
function socketMove(socket,data) {
  ...
  PersistentGame.fromId(gameId,socketErrorHandlerFactory(socket,
    ...
    var ev = { type: 'move', index: pGame.game.moves.length, move: moveObj };

    if (pGame.game.myColor(username) !== moveObj.stone) {

      var color = libgo.longColor(pGame.game.getTurn());
      console.log(pGame.game.getTurn(), color);
