...
function model2Game(model) {
  ...
  var t = model.board && model.board.stones && JSON.parse(model.board.stones);
  console.log(typeof(model.board),s,t);
  //console.log(model);
  var board = libgo.newBoard(s,t);
...
function socketMove(socket,data) {
  ...
  getGameAndSocketErrors(gameId,socket, function (game,model) {
    ...
    var ev = { index: game.moves.length, move: moveObj };
