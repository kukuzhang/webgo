...
function listGames(req,res) {
  ...
  PersistentGame.listGames(req.user,20,0,function (err,docs) {
...
function socketMove(socket,data) {
  ...
  PersistentGame.fromId(gameId,socketErrorHandlerFactory(socket,
	...
	if (pGame.game.myColor(socket.handshake.user) !== move.stone) {

      var color = libgo.longColor(pGame.game.getTurn());
      var player = pGame.game[color] || {};
      var expected = player.name;
      var msg = socket.handshake.user.name + ' is not allowed to play moves of ' + expected;
...
function socketScore(socket,scoring) {
  ...
  var accept = scoring.accept;
  console.log('got score from ' + username + '.',accept);

  PersistentGame.fromId(gameId, socketErrorHandlerFactory(socket,
        function (pGame) {

    console.log('black',pGame.game.scoreOkBlack,'white',pGame.game.scoreOkWhite);
    var state = pGame.game.getState();
    var black = pGame.game.black;
    var white = pGame.game.white;
    var changed = pGame.game.scoreBoard.reduce(function (prev,cur,i) {
        return prev || (cur != scoreBoard[i]);
    }, false);
    ...
    if (sameUser(pGame.game.black, socket.handshake.user)) {
      

      console.log('black', accept);

      pGame.game.scoreOkBlack = accept;

      if (changed) pGame.game.scoreOkBlack = false;

    } else if (sameUser(white, socket.handshake.user)) {

      console.log('white',accept);
      pGame.game.scoreOkWhite = accept;

      if (changed) pGame.game.scoreOkWhite = false;

    } else {

      var msg = username + ' cannot score game of ' +
        black + ' and ' + white + '.';
      console.log(msg);
...
function socketConfigure(socket,configuration) {
  ...
  function (pGame) {
    ...
    if (pGame.game.started) {
      console.log('cannot configure started game.');
